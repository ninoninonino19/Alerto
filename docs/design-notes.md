...
Why Alerto's interface is built the way it is: the layout decisions, the colour
and motion systems, what the automated checks can and cannot reach, and which
decisions here were corrections of earlier ones.

Split out of the README, which now covers what the project is and how to run it.
See [`overview-map.md`](overview-map.md) for a specified-but-unbuilt feature.
