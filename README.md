...
- A single verdict — the current advisory level — at the top of the page.
- One 24-hour chart carrying heat and rainfall together, with the PAGASA bands
  drawn as its axis, so "how far am I from Danger" is read off across the page.
- A "What to do" panel, written from the fixed PAGASA rules and optionally
  rephrased by Gemini into one prioritised list.
- Search for any Philippine locality, or use your device location. Every reading
  has a shareable URL.
...
always says which of the two you are reading. See [Advice generation](#advice-generation)
for what the model is and is not allowed to do.
...
for", never "Observed". The escalation line — the first hour worse than now —
is also a button: press it to jump straight there.
...
The rest of this document is the reasoning behind the interface: why it is laid
out this way, how the colour and motion systems work, and which decisions were
corrections of earlier ones.
